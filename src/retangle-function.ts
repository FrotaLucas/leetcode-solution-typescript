...
function drawRetangle(s: string) : boolean
{
    ...
    let width1 = Math.abs(Math.abs(turns[2][0]) - Math.abs(turns[0][0]));
    let width2 = Math.abs(Math.abs(turns[3][0]) - Math.abs(turns[1][0]));

    let height1 = Math.abs(Math.abs(turns[3][1]) - Math.abs(turns[0][1])); 
    let height2 = Math.abs(Math.abs(turns[2][1]) - Math.abs(turns[0][1]));
